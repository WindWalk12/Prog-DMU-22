...
public class EmployeeWindow extends Stage {
	...
	private void initContent(GridPane pane) {
		...
		Label lblHoursW = new Label("Hourly Wage");
		pane.add(lblHoursW, 0, 2);
	...
	private void okAction() {
		...
		if (name.length() == 0) {
			...
			if (wage < 0 || hours < 0) {
				lblError.setText("Wage or hours is not a positive number");
			} else {
				...
				if (employee != null) {
					...
					if (companyIsSelected) {
						Controller.createEmployee(name, wage, newCompany, employmentYear, hours);
					} else {
